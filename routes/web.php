...
use App\Http\Controllers\MonthlyPlAccrualLogController;
...
use App\Http\Controllers\LatePenaltyLogController;
use Inertia\Inertia;
...
Route::middleware(['auth', 'verified', 'setting'])->group(function () {
    ...
    Route::middleware('plan.access')->group(function () {
        ...
        // Claims Module — permissions checked inside controller methods
        Route::get('hr/claims', [ClaimController::class, 'index'])->name('hr.claims.index');
        Route::post('hr/claims', [ClaimController::class, 'store'])->name('hr.claims.store');
        Route::put('hr/claims/{claim}', [ClaimController::class, 'update'])->name('hr.claims.update');
        Route::delete('hr/claims/{claim}', [ClaimController::class, 'destroy'])->name('hr.claims.destroy');
        Route::get('hr/claims/{claim}', [ClaimController::class, 'show'])->name('hr.claims.show');
        Route::get('hr/claims/{claim}/attachments/{attachment}/view', [ClaimController::class, 'viewAttachment'])->name('hr.claims.attachments.view');
        Route::get('hr/claims/{claim}/attachments/{attachment}/download', [ClaimController::class, 'downloadAttachment'])->name('hr.claims.attachments.download');

        // Claim Approvals (managers/HR/accounts)
        Route::get('hr/claim-approvals', [ClaimController::class, 'approvals'])->name('hr.claim-approvals.index');

        // Claim Workflow Setup (admin)
        Route::get('hr/claim-workflows', [ClaimController::class, 'workflows'])->name('hr.claim-workflows.index');
        Route::post('hr/claim-workflows', [ClaimController::class, 'saveWorkflow'])->name('hr.claim-workflows.store');

        // Company Claim Configuration (admin)
        Route::get('hr/claim-config', [ClaimController::class, 'config'])->name('hr.claim-config.edit');
        Route::put('hr/claim-config', [ClaimController::class, 'saveConfig'])->name('hr.claim-config.update');
        ...
        Route::middleware('permission:manage-wfh-applications|manage-own-wfh-applications|view-wfh-applications|create-wfh-applications')->group(function () {
            Route::get('hr/wfh-applications', [WorkFromHomeRequestController::class, 'index'])->name('hr.wfh-applications.index');
            Route::get('hr/wfh-applications/{workFromHomeRequest}', [WorkFromHomeRequestController::class, 'show'])->middleware('permission:view-wfh-applications')->name('hr.wfh-applications.show');
            Route::post('hr/wfh-applications', [WorkFromHomeRequestController::class, 'store'])->middleware('permission:create-wfh-applications')->name('hr.wfh-applications.store');
            Route::put('hr/wfh-applications/{workFromHomeRequest}', [WorkFromHomeRequestController::class, 'update'])->middleware('permission:edit-wfh-applications')->name('hr.wfh-applications.update');
            Route::delete('hr/wfh-applications/{workFromHomeRequest}', [WorkFromHomeRequestController::class, 'destroy'])->middleware('permission:delete-wfh-applications')->name('hr.wfh-applications.destroy');
            Route::put('hr/wfh-applications/{workFromHomeRequest}/status', [WorkFromHomeRequestController::class, 'updateStatus'])->middleware('permission:approve-wfh-applications')->name('hr.wfh-applications.update-status');
        });
        ...
        Route::middleware('permission:manage-leave-balances')->group(function () {
            ...
            Route::post('hr/leave-balances/monthly-attendance-accrual', [LeaveBalanceController::class, 'processMonthlyAttendanceAccrual'])->middleware('permission:edit-leave-balances')->name('hr.leave-balances.monthly-attendance-accrual');
            Route::get('hr/monthly-pl-accrual-logs', [MonthlyPlAccrualLogController::class, 'index'])->name('hr.monthly-pl-accrual-logs.index');
        });
        ...
        Route::middleware('permission:manage-own-attendance-records|manage-attendance-records')->group(function () {
            Route::get('hr/my-attendance-register', [AttendanceRecordController::class, 'employeeMonthlyAttendance'])->name('hr.my-attendance-register.index');
        });
        ...
        Route::middleware('permission:manage-attendance-records')->group(function () {
            ...
            Route::get('hr/muster-roll', [AttendanceRecordController::class, 'musterRoll'])->name('hr.muster-roll.index');

            Route::get('hr/attendance-records/export', [AttendanceRecordController::class, 'export'])->name('hr.attendance-records.export');
            Route::post('hr/muster-roll/close-month', [AttendanceRecordController::class, 'closeMusterMonth'])->name('hr.muster-roll.close-month');
            Route::post('hr/muster-roll/reopen-month', [AttendanceRecordController::class, 'reopenMusterMonth'])->name('hr.muster-roll.reopen-month');
        ...
        // HR Notifications (Admin section)
        Route::get('hr/notifications', [HrNotificationController::class, 'index'])->name('hr.notifications.index');
        Route::delete('hr/notifications', [HrNotificationController::class, 'bulkDestroy'])->name('hr.notifications.bulk-destroy');
        Route::delete('hr/notifications/{notification}', [HrNotificationController::class, 'destroy'])->name('hr.notifications.destroy');

        // Employee Daily Timesheet routes
        Route::middleware('permission:manage-own-time-entries')->group(function () {
            Route::get('hr/daily-timesheets/form', [DailyTimesheetController::class, 'form'])->name('hr.daily-timesheets.form');
            Route::post('hr/daily-timesheets/draft', [DailyTimesheetController::class, 'saveDraft'])->name('hr.daily-timesheets.save-draft');
            Route::post('hr/daily-timesheets/submit', [DailyTimesheetController::class, 'submitForApproval'])->name('hr.daily-timesheets.submit');
            Route::get('hr/daily-timesheets/my', [DailyTimesheetController::class, 'myIndex'])->name('hr.daily-timesheets.my.index');
        });

        Route::middleware('permission:approve-time-entries')->group(function () {
            Route::get('hr/daily-timesheets/approvals', [DailyTimesheetController::class, 'approvalsIndex'])->name('hr.daily-timesheets.approvals.index');
            Route::put('hr/daily-timesheets/{timesheet}/status', [DailyTimesheetController::class, 'updateStatus'])->name('hr.daily-timesheets.update-status');
        });

        Route::middleware('permission:manage-own-time-entries|approve-time-entries')->group(function () {
            Route::delete('hr/daily-timesheets/{timesheet}', [DailyTimesheetController::class, 'destroy'])->name('hr.daily-timesheets.destroy');
        });
        ...
        Route::get('hr/late-penalty-logs', [\App\Http\Controllers\LatePenaltyLogController::class, 'index'])
            ->middleware('permission:manage-attendance-records')
            ->name('hr.late-penalty-logs.index');
        Route::post('hr/late-penalty-logs/calculate', [\App\Http\Controllers\LatePenaltyLogController::class, 'calculate'])
            ->middleware('permission:manage-attendance-records')
            ->name('hr.late-penalty-logs.calculate');
