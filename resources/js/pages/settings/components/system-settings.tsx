...
export default function SystemSettings({
  ...
  const defaultSettings: Record<string, any> = {
    ...
    allowBackdatedAttendanceRequests: true,
  ...
  const [systemSettings, setSystemSettings] = useState(() => ({
    ...
    allowBackdatedAttendanceRequests: settingsData.allowBackdatedAttendanceRequests === '1' || settingsData.allowBackdatedAttendanceRequests === 1 || settingsData.allowBackdatedAttendanceRequests === true || (settingsData.allowBackdatedAttendanceRequests === undefined ? defaultSettings.allowBackdatedAttendanceRequests : false),
    ...
    companyWebsiteUrl: settingsData.companyWebsiteUrl || defaultSettings.companyWebsiteUrl
  }));
  ...
  useEffect(() => {
    if (Object.keys(settingsData).length > 0) {
      ...
      const mergedSettings = Object.keys(defaultSettings).reduce((acc: Record<string, any>, key) => {
      ...
      setSystemSettings(prevSettings => ({
        ...
        allowBackdatedAttendanceRequests: mergedSettings.allowBackdatedAttendanceRequests === '1' || mergedSettings.allowBackdatedAttendanceRequests === 1 || mergedSettings.allowBackdatedAttendanceRequests === true || (mergedSettings.allowBackdatedAttendanceRequests === undefined ? defaultSettings.allowBackdatedAttendanceRequests : false),
        ...
        companyWebsiteUrl: mergedSettings.companyWebsiteUrl || defaultSettings.companyWebsiteUrl
      }));
    }
  }, [settingsData]);
  ...
  const submitSystemSettings = (e: React.FormEvent) => {
    ...
    const cleanSettings: any = {
      ...
      allowBackdatedAttendanceRequests: Boolean(systemSettings.allowBackdatedAttendanceRequests),
      userRegistrationEnabled: systemSettings.userRegistrationEnabled ? 1 : 0,
      termsConditionsUrl: systemSettings.termsConditionsUrl,
      companyWebsiteUrl: systemSettings.companyWebsiteUrl
    };
    ...
    if (isCompanyUser) {
      ...
      cleanSettings.allowBackdatedAttendanceRequests = Boolean(systemSettings.allowBackdatedAttendanceRequests);
      cleanSettings.companyWebsiteUrl = systemSettings.companyWebsiteUrl;
    }
          ...
          {isCompanyUser && (
            <>
              ...
              <div className="grid gap-2 md:col-span-2">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <Label htmlFor="allowBackdatedAttendanceRequests">{t("Allow Backdated Attendance Requests")}</Label>
                    <p className="text-sm text-muted-foreground">
                      {t("Allow employees to apply attendance regularization and leave for past dates.")}
                    </p>
                  </div>
                  <Switch
                    id="allowBackdatedAttendanceRequests"
                    checked={Boolean(systemSettings.allowBackdatedAttendanceRequests)}
                    onCheckedChange={(checked) => handleSystemSettingsChange('allowBackdatedAttendanceRequests', checked)}
                  />
                </div>
              </div>
              <div className="grid gap-2 md:col-span-2">
                <Label htmlFor="companyWebsiteUrl">{t("Company Website URL")}</Label>
                <Input
                  id="companyWebsiteUrl"
                  type="url"
                  placeholder="https://example.com"
                  value={systemSettings.companyWebsiteUrl}
                  onChange={(e) => handleSystemSettingsChange('companyWebsiteUrl', e.target.value)}
                />
                <p className="text-sm text-muted-foreground">
                  {t("This URL is used on the public Career page logo click action.")}
                </p>
              </div>
            </>
          )}
