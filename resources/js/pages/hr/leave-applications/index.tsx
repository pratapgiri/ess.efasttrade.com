...
export default function LeaveApplications() {
  ...
  const handleFormSubmit = (formData: any) => {
    const isHalfDay = formData?.is_half_day === true || formData?.is_half_day === '1' || formData?.is_half_day === 1;
    const payload = {
      ...formData,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_value: string, row: any) => {
        if (Number(row.total_days) !== 0.5) return <span className="text-muted-foreground">—</span>;
        if (row.half_day_part === 'second_half') return <span className="text-sm">{t('Second half')}</span>;
        if (row.half_day_part === 'first_half') return <span className="text-sm">{t('First half')}</span>;
    ...
    >
      ...
      <CrudFormModal
        ...
        formConfig={{
          fields: [
            ...
            },
            {
              ...
              required: true,
              options: [
                { value: '0', label: t('Full Day') },
                { value: '1', label: t('Half Day') },
              ]
            },
            {
              name: 'half_day_part',
              ...
              options: [
                { value: 'first_half', label: t('First half (AM / before break)') },
                { value: 'second_half', label: t('Second half (PM / after break)') },
              ],
              conditional: (_mode: string, fd: any) =>
                fd.is_half_day === '1' || fd.is_half_day === true || fd.is_half_day === 1,
            },
        ...
        initialData={currentItem ? {
          ...currentItem,
          is_half_day: Number(currentItem.total_days) === 0.5 ? '1' : '0',
          half_day_part: currentItem.half_day_part || 'first_half',
        ...
        } : { is_half_day: '0', half_day_part: 'first_half' }}
        title={
          formMode === 'create'
