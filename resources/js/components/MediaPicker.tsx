...
export default function MediaPicker({
  ...
  const handleSelect = (selectedUrl: string) => {
    ...
    // Keep storage-relative path so nested media folders still resolve correctly.
    if (selectedUrl.includes('/storage/')) {
      const storageRelative = selectedUrl.split('/storage/')[1];
      onChange(storageRelative || selectedUrl);
      return;
    }

    const mediaIndex = selectedUrl.indexOf('media/');
    if (mediaIndex !== -1) {
      onChange(selectedUrl.substring(mediaIndex));
      return;
    }
