...
export default function CareerHeader({ title, showBackToHome, logoOnly = false, companySettings }: CareerHeaderProps) {
  ...
  const getThemeColor = () => {
    ...
    const colorKey = String(themeColor) as keyof typeof colorMap;
  ...
  const getCompanyWebsiteUrl = () => {
    const rawUrl =
      companySettings?.companyWebsiteUrl ||
      companySettings?.company_website_url ||
  ...
  const companyWebsiteUrl = getCompanyWebsiteUrl();
  const isExternalWebsite = /^https?:\/\//i.test(companyWebsiteUrl);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-center items-center h-12 sm:h-16">
            <a
              ...
              aria-label={`${getCompanyName()} website`}
            >
              <img
