...
const getImagePath = (path: string, pageProps?: any): string => {
  ...
  const normalizedMediaPath = normalizedInput
    .replace(/^storage\/+/, '')
    .replace(/^app\/public\/+/, '')
    .replace(/^public\/+/, '')
    .replace(/^media\/+/, '');
  ...
  if (normalizedInput.startsWith('storage/')) {
    return `${window.location.origin}/${normalizedInput}`;
  }

  if (normalizedInput.startsWith('app/public/')) {
    return `${window.location.origin}/storage/${normalizedInput.replace(/^app\/public\/+/, '')}`;
  }

  if (normalizedInput.startsWith('public/')) {
    return `${window.location.origin}/storage/${normalizedInput.replace(/^public\/+/, '')}`;
  }

  if (normalizedInput.startsWith('media/')) {
    return `${window.location.origin}/storage/${normalizedInput}`;
  }
  ...
  try {
    ...
    return `${window.location.origin}/storage/media/${normalizedMediaPath}`;
  }
}
