...
class LeaveBalanceController extends Controller
{
    ...
    /**
     * Manual / back-fill trigger of the monthly leave accrual for a single
     * leave type. The automatic run is handled by the scheduled
     * `leave:monthly-accrual` command.
     *
     * Rule (current): credits a flat 2 days/month per active or probation
     * employee, pro-rated by calendar days for those who joined inside the
     * month being processed, and then clamps any negative closing balance to
     * zero (overdrawn portion is already tagged as LWP on the leave
     * application and is deducted by payroll, so no double deduction).
     */
    public function processMonthlyAttendanceAccrual(Request $request, MonthlyPaidLeaveAccrualService $accrualService)
    {
        if (! Auth::user()->can('edit-leave-balances')) {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
        ...
        $viewer = Auth::user();
        $companyIdForLog = $viewer->type === 'company'
            ? (int) $viewer->id
            : (int) ($viewer->created_by ?: 0);
        if ($companyIdForLog <= 0) {
            return redirect()->back()->with('error', __('Unable to determine company for accrual log.'));
        }

        try {
            $result = $accrualService->processMonth(
                (int) $validated['year'],
                (int) $validated['month'],
            ...
            return redirect()->back()->with(
                'success',
                __('Monthly leave accrual completed for :month/:year. Employees credited: :count. Closing-balance clamps: :clamped. Log batch: :batch.', [
                    'month' => str_pad((string) $result['month'], 2, '0', STR_PAD_LEFT),
                    'year' => $result['year'],
