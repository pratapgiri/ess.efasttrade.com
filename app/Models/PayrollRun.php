...
class PayrollRun extends BaseModel
{
    ...
    private function getEmployeeLeaveData($employeeId)
    {
        ...
        foreach ($leaveApplications as $leave) {
            ...
            $isFullyInsidePeriod =
                $leave->start_date->betweenIncluded($this->pay_period_start, $this->pay_period_end) &&
                $leave->end_date->betweenIncluded($this->pay_period_start, $this->pay_period_end);

            $leaveDays = $isFullyInsidePeriod
                ? (float) $leave->total_days
                : (float) ($leaveStart->diffInDays($leaveEnd) + 1);

            $lwpFromTag = 0.0;
            $comments = (string) ($leave->manager_comments ?? '');
            if (preg_match('/\[LWP_DAYS:([0-9]+(?:\.[0-9]+)?)\]/', $comments, $matches)) {
                $lwpFromTag = (float) $matches[1];
            }

            if ($leave->leaveType->is_paid) {
                $lwpPortion = min($leaveDays, max(0, $lwpFromTag));
                $paidLeaveDays += max(0, $leaveDays - $lwpPortion);
                $unpaidLeaveDays += $lwpPortion;
            } else {
