            ...
            if (isSaas() && $user->type === 'company' && ! $user->referral_code) {
                ...
                static::created(function ($createdUser) {
                    if (! $createdUser->referral_code) {
                        ...
                        //$createdUser->save();
                        static::where('id', $createdUser->id)->update([
                           // 'referral_code' => 'REF'.str_pad($createdUser->id, 6, '0', STR_PAD_LEFT),
                            'referral_code' => (int) str_pad((string) $createdUser->id, 6, '0', STR_PAD_LEFT),
                        ]);
                    }
                });
            }
